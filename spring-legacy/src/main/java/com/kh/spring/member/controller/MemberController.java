...
public class MemberController {
	 ...
	 *    스프링이 생성한 객체(bean)을 주입받아서 생성하는 방식
	 *    
...
//	}
	 ...
	 *  - 클라이언트가 요청한 값을 대신 변환하여 바인딩 해주는 역할을 argumentResolver가 수행한다.
	 * */
//	public String login(@RequestParam(value="userId", defaultValue="pmh")String userId) {
...
//	}
	 ...
	 *  - 요청시 전달한 name속성과 일치하는 vo클래스 필드에 setter메서드를 호출하여 데이터를 바인딩한다.
...
//	}
	...
	public ModelAndView login(
		...
		if(loginUser != null) {
			...
			/* 
			 * RedirectAttributes의 flashgAttribute는 데이터를 우선
			 * SessionScope에 담았다가, 리다이렉트가 완료되면 SessionScope의 데이터를 requestScope로 변경해준다.
			 * */
		}
	...
	public String logout(HttpSession session, SessionStatus status) {
		...
		// session.invalidate(); // model로 sessionScope에 이관된 데이터를 삭제하는 메서드
		status.setComplete();
